...
public static class SavePkmHelper
{
    ...
    public static void ReadSaveData(string filePath)
    {
        ...
        {
            byte[] info = new UTF8Encoding(true).GetBytes(trainerString);
        ...
        foreach (var poke in sav.BoxData)
        {
            ...
            using (FileStream fs = File.Create("box"+counter+".txt"))
            {
                byte[] info = new UTF8Encoding(true).GetBytes(boxString);
                fs.Write(info, 0, info.Length);
